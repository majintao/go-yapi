...
func (s *InterfaceService) AddOrUpdate(data *InterfaceData) (*ModifyResp, *http.Response, error) {
	apiEndpoint := "api/interface/save"
	addOrUpdateInterfaceData := new(AddOrUpdateInterfaceData)
	addOrUpdateInterfaceData .Token = s.client.Authentication.token

	req, err := s.client.NewRequest("POST", apiEndpoint, addOrUpdateInterfaceData)
	if err != nil {
		return nil, nil, err
	}
	req.Header.Add("Content-Type", "application/json")
...
func (s *InterfaceService) UploadSwagger(data *string) (*ModifyResp, *http.Response, error) {
	...
	req, err := s.client.NewRequest("POST", apiEndpoint, uploadSwaggerReq)
	if err != nil {
		return nil, nil, err
	}
	req.Header.Add("Content-Type", "application/json")
